...
async def test_agent_nodes_with_mocks() -> None:
    ...
    with patch("marketplace_matching_agent.agents.fairness.append", new=AsyncMock(return_value="hash")):
...
async def test_graph_node_wrappers() -> None:
    ...
    with patch("marketplace_matching_agent.agents.fairness.append", new=AsyncMock(return_value="h")):
...
async def test_cohere_rerank_with_api_key() -> None:
    mock_result = type("R", (), {"results": [type("X", (), {"index": 0, "relevance_score": 0.99})()]})()
    with patch("marketplace_matching_agent.retrieval.rerank.get_settings") as mock_settings:
        ...
        with patch("marketplace_matching_agent.retrieval.rerank.cohere.AsyncClientV2") as mock_client:
