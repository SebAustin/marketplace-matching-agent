"""Ingest resumes into Qdrant and Tantivy."""
...
async def main() -> None:
    fixture = Path("tests/fixtures/candidates_docs.json")
    docs = json.loads(fixture.read_text())
    index_documents("candidates", docs)
    settings = get_settings()
    client = AsyncQdrantClient(url=settings.qdrant_url)
    await _ensure_collection(client, "candidates")
    points = []
    ...
    await client.upsert(collection_name="candidates_v1", points=points)
    log.info("ingest_resumes_done", n=len(docs))
